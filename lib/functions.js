...
// Fonction de récupération du token et vérification à l'aide de JWT
...
// Fonction de comparaison du mot de passe avec le hash
...
// Fonction de neutralisation des caractères spéciaux à l'aide de la librairie validator
function sanitizeInput(input) {
  return validator.escape(input);
}

// Fonction dédiée aux champs email et username
function prepareEmailAndUsername(email, username) {
  const sanitizedEmail = sanitizeInput(email);
  const sanitizedUsername = sanitizeInput(username);
  const emailLowerCase = sanitizedEmail.toLowerCase();
...
// Fonction de calcul de la note moyenne des posts sur un restaurant
